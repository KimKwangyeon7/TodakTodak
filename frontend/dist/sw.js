...
}, C = (a) => [f.prefix, a, f.suffix].filter((e) => e && e.length > 0).join("-"), x = (a) => {
...
async function A(a, e) {
  ...
  }, r = e ? e(n) : n, i = W() ? s.body : await s.blob();
...
class B {
  ...
  async cachePut(e, t) {
    ...
    if (!r)
      return !1;
    const { cacheName: i, matchOptions: c } = this._strategy, o = await self.caches.open(i), h = this.hasCallback("cacheDidUpdate"), g = h ? await q(
...
self.addEventListener("push", function(a) {
  a.data ? data = JSON.parse(a.data.text()) : console.log("데이터가 비어있습니다!");
  const e = {
    body: data.pMsg,
  ...
  a.waitUntil(
    // 알림 메시지 전송하여 표시
    self.registration.showNotification(data.pTitle, e)
  );
});
self.addEventListener("notificationclick", function(a) {
  a.action == "like" ? clients.openWindow("https://www.starbucks.com/") : a.action == "info" && clients.openWindow("https://code-design.web.app/"), a.notification.close();
}, !1);
